...
at the league's own rate — came out at **0.884**. Comfortably below 1.0, interval excluding it.
Benfica sat at 0.883. On its face: two clubs booked around 12% less than their fouls imply.

That is roughly the shape the original claim predicted. It is also wrong, three times over.

## Wrong (1): most of it is being good, and it applies to everyone

![Context effect](figures/fig1-context-effect.png)

Across all eleven leagues, booking rate per foul depends heavily on how strong you were expected to
be in that specific match. A heavy underdog is booked at **1.122** times its foul count; a heavy
favourite at **0.851**. A 32% swing, and no club identity is involved — this is every team, in
every league, purely as a function of the situation it is in.

Dominant clubs are favourites most weeks. So roughly half of Porto's apparent leniency was simply
...
| | |
|---|---|
| corr(club strength, **own** booking index) | **+0.486** |
| corr(club strength, **opponents'** booking index) | **+0.284** (p = 6×10⁻⁶) |
| corr(own index, opponents' index) | +0.115 (p = 0.07) |

When a dominant club plays, **the other team is booked more per foul too** — and the two are
close to independent. A club-level story predicts the second correlation is zero. It is not.
...
**65% of 246 clubs** are booked more per foul when they face better opposition. Porto's opponents
run at 1.155; Celtic's at 1.057.
...
cards *for the same number of fouls*". Rediscovering a known result on 43 times the data is worth
...
the arithmetic is identical, and so is the error.

## Tri-anchor

| Anchor | Source |
|---|---|
| **Data** | 116,262 team-matches, 11 leagues, 2000–2026, committed snapshot `2026-W35`; plus 2,737 Portuguese match officials |
...
real driver is absolute quality, that would explain why the gradient is strongest in Spain (+0.63)
and weakest or reversed in Scotland. Testable with UEFA country coefficients, and next.
...
```bash
uv run python scripts/strength_effect.py
